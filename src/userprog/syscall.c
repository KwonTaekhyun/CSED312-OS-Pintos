...
syscall_handler (struct intr_frame *f) 
{
  ...
  int syscall_nr;
  if(read_argument(SP, &syscall_nr, sizeof(int)) == -1){
    exit(-1);
  }
  ...
  switch(syscall_nr){
    ...
    case SYS_EXIT: {
      ...
      // 1. return state 읽어오기
      int exit_status;
      if(read_argument(SP + 4, &exit_status, sizeof(int)) == -1){
        exit(-1);
      }
    ...
    case SYS_CREATE: {
      ...
      if(read_argument(SP + 4, file, sizeod(uint32_t)) == -1){
        *RT = false;
        exit(-1);
      }
      if(read_argument(SP + 8, &initial_size, sizeod(unsigned)) == -1){
        *RT = false;
        exit(-1);
      }
